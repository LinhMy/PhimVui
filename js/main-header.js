...
function ajax_movies_block(url, data, callback, error_callback) {
    ...
    if (localCache.exist(cache_key)) {
        ...
        $.ajax({url: https_url + url, type: 'get', cache: true, data: data}).done(function (html) {
...
function top_view_country_by_type(country, type, option) {
    $.ajax({
        url: https_url + 'ajax/top_view_country_by_type',
        type: 'POST',
        dataType: 'json',
        data: 'country=' + country + '&top=' + option + '&type=' + type,
    }).done(function (data) {
        if (data.html != null || data.html != '') {
            $('#widget_top_film_country_by_type_' + type).html(data.html);
            $("ul.film").init_scollbar_inset_dark();
        }
    }).fail(function (e) {
        console.log('error-connection', e);
